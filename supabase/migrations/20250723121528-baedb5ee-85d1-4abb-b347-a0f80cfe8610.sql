...
-- 8. Create a dedicated role for the function (security improvement)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'profile_manager') THEN
    CREATE ROLE profile_manager;
    GRANT INSERT, UPDATE ON public.profiles TO profile_manager;
  END IF;
END $$;

-- 9. Update the handle_new_user function with improved error handling
...
-- 10. Set the function owner to the limited role
ALTER FUNCTION public.handle_new_user() OWNER TO profile_manager;