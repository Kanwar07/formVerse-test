...
const SafeModelRenderer = ({
  ...
  return (
    <group
      scale={[
        scaledModel.scaleFactor,
        scaledModel.scaleFactor,
        scaledModel.scaleFactor,
      ]}
      position={[
        scaledModel.center.x,
        scaledModel.center.y,
        scaledModel.center.z,
      ]}
    >
      <mesh
...
export const UnifiedCADViewer: React.FC<UnifiedCADViewerProps> = ({
  ...
  const loadModel = useCallback(async () => {
    try {
      ...
      console.log("UnifiedCADViewer: Starting to load model", {
        fileUrl,
        fileName,
        detectedFileType,
      });
      ...
      const loadedModel = await EnhancedCADLoader.loadModel(
        ...
        (progress) => {
          console.log(
            `Loading progress: ${progress.progress}% - ${progress.stage}`
          );
        }
      );
      ...
      const info: ModelInfo = {
        ...
        fileSize: loadedModel.metadata?.fileSize
          ? `${(loadedModel.metadata.fileSize / 1024).toFixed(1)} KB`
          : undefined,
      };
      ...
      toast({
        ...
        description: `${
          info.format
        } file with ${info.vertices.toLocaleString()} vertices`,
      });
    } catch (err) {
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-background/50 backdrop-blur-sm">
              <div className="flex flex-col items-center space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Loading {detectedFileType.toUpperCase()} model...
                </p>
              </div>
            </div>
          )}
        ...
        <div className="mt-4">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-sm text-muted-foreground mt-2">
                Watch how your image was transformed into a 3D model using
                CADQUA AI.
              </p>
            </CardContent>
          </Card>
