...
import { ImagePlus, Info, Lightbulb } from "lucide-react";
...
export default function ImageToCADFlow({ handleGenerate }) {
  const [quality, setQuality] = useState("");
  return (
    <div className="p-8 w-full sm:w-[400px] md:w-[530px] md:h-[568px] bg-[#001F4C]/50 rounded-[20px] flex flex-col gap-4">
      <div className="flex flex-col gap-2 mt-4">
        <div className="flex flex-row justify-between">
          <div className="text-[14px] font-bold">Image</div>
          <div className="flex flex-row gap-2">
            <Lightbulb size={16} className="cursor-pointer" />
            <Info size={16} className="cursor-pointer" />
          </div>
        </div>
        <div className="flex flex-row items-center gap-5 justify-center bg-[#D9D9D9]/10 rounded-[8px] px-6 py-4">
          <ImagePlus size={24} />
          <div className="flex flex-col">
            <span className="text-[14px] font-bold">
              Drag and drop/ Paste Image
            </span>
            <span className="text-[12px] font-normal text-[#FFFFFF]/60">
              .png, .jpg, .jpeg, .webp | Max size : 20 MB
            </span>
          </div>
        </div>
      </div>
